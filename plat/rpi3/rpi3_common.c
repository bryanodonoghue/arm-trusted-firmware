...
{
	/*
	 * The only thing that can cause a secure interrupt in this platform is
	 * the secure timer.
	 */
	u_register_t cntps_ctl_el1 = read_cntps_ctl_el1();

	if ((get_cntp_ctl_enable(cntps_ctl_el1) != 0U) &&
	    (get_cntp_ctl_istatus(cntps_ctl_el1) != 0U)) {
				     ...
				     uint32_t security_state)
{
	...
	/*
	 * Currently there are no secure interrupts but we allow this function to
	 * return FIQ line anyway.
	 */
	WARN("rpi3: Secure interrupts not supported\n");
